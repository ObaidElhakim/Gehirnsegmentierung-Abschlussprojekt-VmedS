...
def load_volume_corrected(path, name):
    ...
    so, dass die Slices ähnlich aussehen wie bei deinem 2D-Code.
    """
    print(f"\n--- Lade Volumen {name} ---")
    img = nib.load(path)
...
IR_vol    = load_volume_corrected("pat13_reg_IR.nii.gz",    "IR")  # ggf. IR-Datei anpassen

nx, ny, nz = T1_vol.shape
...
brain_mask_3d = np.zeros_like(T1_vol, dtype=bool)
...
for z in range(nz):
    ...
    # intensity-basiertes Skull-Stripping wie bei dir
    brain_mask_filtered = brain_mask_combined & (T1_slice < t1_upper_threshold)
    brain_mask_filtered = brain_mask_filtered & (FLAIR_slice > flair_lower_threshold)
    brain_mask_stripped = binary_fill_holes(brain_mask_filtered)
...
brain_idx = np.where(brain_mask_3d)
if brain_idx[0].size == 0:
    raise RuntimeError("3D-Brain-Maske ist leer – überprüfe Thresholds und Daten.")

feat_T1    = T1_vol[brain_idx]
feat_FLAIR = FLAIR_vol[brain_idx]
feat_IR    = IR_vol[brain_idx]
